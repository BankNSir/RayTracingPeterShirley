    ...
    for (int i = -7; i < 7; i++)
    {
        for (int j = -7; j < 7; j++)
        {
            ...
            {    
                ...
                {
                    listOfObject[count++] = new sphere(center, 0.3, new lambertian(vec3(random()*random(), random()*random(), random()*random())));
                }
                else if (material_selector < 0.8) // matal
                {
                    listOfObject[count++] = new sphere(center, 0.3, new metal(vec3(0.5 * (1 + random()), 0.5 * (1 + random()), 0.5 * (1 + random()))));
                }
                else // dielectric
                {
                    listOfObject[count++] = new sphere(center, 0.3, new dielectric(1.5));
                }
            }   
        }    
    }
...
{
    fstream file;
    file.open("./All Picture/Ch13.PPM", ios::out | ios::trunc);
    ...
    camera cam(lookfrom, lookat, vec3(0, 1, 0), 45, float(nx)/float(ny), aperture, focus_dist);
    // camera cam;
